...
legacy の **`subject` タスク**（`--task subject`）は代わりに
`SUBJECTS` × `ADJECTIVES` × `SETTINGS` × `TEMPLATES`
（例: `"a fluffy photo of a cat on a wooden table"`）を組み合わせ、
固定の 被写体→ペルソナ マップでラベリングします。
...
**評価条件** — 本パイプラインが既定の **`preference` タスク（`gamma=2.0`）** で生成する
合成テキスト→画像検索の評価セット: **コーパス 200 画像・クエリ 200 件**（7 ペルソナ）。
各クエリの正解は **argmax-appeal ペルソナが一致する*全画像***（ペルソナあたり 10〜59 枚・
平均 ≈29 枚）のため、完全な並び替えでも Recall@1 の絶対値は低くなります。デフォルトの
...
|---|---|---|---|---|---|---|
| ベース (Qwen3-VL-Embedding-2B) | 0.190 | 0.765 | 0.133 | 0.273 | 0.106 | 0.035 |
| ファインチューニング後 | **0.530** | **1.000** | **0.647** | **0.703** | **0.492** | **0.208** |

*Acc@k = 上位 k に正解画像が 1 枚以上入るクエリの割合。* わずか 1 エポックの FT で
NDCG@10 は **0.133 → 0.647** に向上します。ベース Acc@1 = 0.19 は 7 ペルソナの
ランダム水準（≈0.143）に近く、FT 無しでは解けないタスクであることを示します。legacy の
`subject` タスク（NDCG@10 ≈ 0.985 で飽和）と違い、preference タスクは埋め込みを
**天井未満**に留めます——これがリランカーの効く余地そのものです。
...
|---|---|---|---|---|---|---|
| ベース | なし | 0.190 | 0.089 | 0.122 | 0.273 | 0.030 |
| ベース | ベース | 0.000 | 0.052 | 0.094 | 0.131 | 0.030 |
| ベース | FT | 0.000 | 0.108 | 0.106 | 0.167 | 0.030 |
| FT | なし | 0.530 | 0.644 | 0.646 | 0.703 | 0.208 |
| FT | ベース | 0.760 | 0.639 | 0.654 | 0.820 | 0.208 |
| **FT** | **FT** | **0.790** | **0.696** | **0.680** | **0.860** | 0.208 |

今度はリランカが**効きます**: FT 埋め込みの上に FT リランカを載せると
**MRR が 0.703 → 0.860（+0.157）**、**NDCG@1 が 0.530 → 0.790（+0.26）**。これは
preference タスクの非加法構造の成果で、リランクが悪化するだけだった legacy subject タスクの
