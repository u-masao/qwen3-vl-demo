...
- **Experiment**:
  ...
  - `evaluate` … `evaluate`（base / finetuned）と `rerank` の各バリアントを記録。Retriever
    （`rerank=none`）と Reranker（`rerank=base/ft`）が同じ土俵の run として並ぶ。rerank は
    stage 全体を親 run、各バリアントを子 run（nested）にする。
  - `train` … `train` / `train_reranker` の学習を記録。
