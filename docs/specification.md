...
- 知識蒸留（teacher＝リランカー or 嗜好モデル → student＝埋め込み bi-encoder）と蒸留後評価
- 結果の可視化（Gradio）
...
知識蒸留（`distill.py`）の **teacher** は上表に新しいモデルを追加しません。`teacher=reranker` は
上記 FT 済みリランカー（cross-encoder）を再利用し、`teacher=oracle` は `preference.py` の嗜好モデル
（＝正解ラベルの作り手。`preference_model.json` だけで動きモデルロード不要）を使います。**student** は
常に埋め込み bi-encoder で、初期化元は埋め込みモデル（ベース／FT 済み）です。
...
first-level キー（`common` / `data` / `preference` / `image_gen` / `embedding` / `reranker` /
`train` / `distill` / `distill_variants`）でセクション分けします。パスはリポジトリルートからの
相対で記述します。
...
| `embedding.query_prompt_name` | str\|null | `default` | クエリ用 instruction prompt 名。`default` = `"Represent the user's input."` で base/FT/蒸留の全モデルに一貫適用（Issue #20。smoke は `null`） |
...
| `distill.teacher` | str | `reranker` | 蒸留 teacher。`reranker`（パターン A: cross→bi, MarginMSE）/ `oracle`（パターン B: 嗜好モデルの soft label, CoSENT） |
| `distill.model_dir` | str | `outputs/model_distilled` | 蒸留済み student 埋め込みの保存先（`make distill` 単発実行時） |
| `distill.num_negatives` | int | `3` | 蒸留ペアの 1 クエリあたり負例数（ハードネガティブマイニング。Issue #25 では 7 で検証。15 は VRAM スピルで OOM） |
| `distill.temperature` | float | `1.0` | oracle soft relevance の温度（`reranker` teacher では未使用） |

蒸留先（student）を複数パターン試す `distill_variants` は、`<name>` をキーとするマトリクスです。
DVC の `distill@<name>` / `eval_distill@<name>`（`foreach` ステージ）が読み、variant ごとに独立
キャッシュします（出力 `outputs/distill_<name>`、メトリクス `metrics_distill_<name>.json`）。

| `distill_variants.<name>.*` | 値 | 意味 |
|---|---|---|
| `teacher` | `reranker` / `oracle` | teacher（パターン A / B） |
| `student_model` | `none` / `ft` / 任意 ID・パス | student の初期化元。`none`=ベース埋め込み（自己蒸留）/ `ft`=FT 済み埋め込みから継続 / それ以外=小型 cross-modal 埋め込みへ圧縮 |
| `student_kind` | `bi`（対応済み）/ `cross`（今後） | bi-encoder（検索）か cross-encoder（リランカー）圧縮か。`cross` は現状 `NotImplementedError` |
| `quantize` | `none`（対応済み）/ `8bit` / `4bit`（今後） | 量子化自己蒸留（QLoRA, GPU 限定）。`none` 以外は現状 `NotImplementedError` |

既定 `params_default.yaml` には `self` / `ft_continue` / `oracle_base` / `oracle_ft` の 4 variant が
定義されています（teacher × student 初期値の 2×2）。
...
| `python -m qwen3vl_demo.distill` | `--distill-teacher`, `--distill-student-model`, `--distill-student-kind`, `--distill-quantize`, `--distill-num-negatives`, `--distill-temperature`, `--distill-model-dir` | 知識蒸留（teacher → student 埋め込み）。`reranker` teacher で `reranker.model_id` が null ならスキップ |
...
| `<distill.model_dir>/`（既定 `outputs/model_distilled/`） | distill | 蒸留済み student 埋め込み（SentenceTransformer）。`make distill` 単発実行時 |
| `<output_dir>/metrics_distilled.json` | evaluate (--label distilled) | 蒸留後 student の検索メトリクス（`make eval-distill`） |
| `<output_dir>/distill_<name>/` | distill@\<name\> (DVC) | variant 別の蒸留済み student（`distill_variants` 展開時） |
| `<output_dir>/metrics_distill_<name>.json` | eval_distill@\<name\> (DVC) | variant 別の蒸留後メトリクス |
...
評価・学習の結果は [MLflow](https://mlflow.org/) に記録します（`src/qwen3vl_demo/tracking.py`。Issue #9）。

- **バックエンド**: SQLite3（`sqlite:///<repo>/mlflow.db`）。`MLFLOW_TRACKING_URI` で差し替え可能。
- **artifact**: `<repo>/mlflow/<exp_id>/<run_id>/artifacts`（MLflow 既定風レイアウト）。
- **run の単位/期間**: CLI（DVC ステージ）1 実行 = 1 run。run は各 `main()` の `cli_run` が
  引数解決直後に開き終了直前に閉じるので、モデルロード・データ準備を含む CLI 全体を覆う
  （System Metrics・所要時間が全工程をカバー）。例外的に rerank はこの run を親とし、
  バリアント比較用の子 run（nested）をぶら下げる。
- **Experiment**:
  - `generate_data` … データ生成を記録（所要時間・キャッシュのヒット/ミス・生成枚数）。
  - `evaluate` … `evaluate`（base / finetuned）・`rerank`・`eval_distill`（蒸留後 student）の各
    バリアントを記録。Retriever（`rerank=none`）と Reranker（`rerank=base/ft`）が同じ土俵の run
    として並ぶ。rerank は stage 全体を親 run、各バリアントを子 run（nested）にする。
  - `train` … `train` / `train_reranker` / `distill` の学習を記録（distill は `teacher` タグ付き）。
- **params**: 起動引数を `args.*`、解決後の全設定を `cfg.*` として漏れなく記録。
- **metrics**: 評価器の返り値（NDCG / Recall@k / MRR 等。`@`→`_at_` にサニタイズ）、
  各工程の所要時間 `time.*_sec`、リランクのピーク VRAM `vram.*_peak_mib`。
- **学習曲線**: 学習中の loss / eval 指標を step 付きで記録（`make_curve_callback`）。
- **System Metrics**: CPU / メモリ / GPU 使用率・VRAM の時系列を自動収集（`psutil` / `nvidia-ml-py`）。
...
`mlflow.db` と `mlflow/`（artifact）は `.gitignore` 済み。実験はコード／DVC から再現できます。
