...
│ (キャプション)│   + SD-Turbo  │   .py         │   train / eval    │  (ディスク)   │
└─────────────┘               └──────────────┘                   └──────┬───────┘
                                                                         │
                          ┌──────────────────────────────────────────────┤
                          ▼                                              ▼
...
| `rerank.py` | 埋め込み検索 top-k を Reranker で再ランク | `data*/eval`, モデル | `rerank_examples.json` |
...
| カラム | 型 | 意味 |
|---|---|---|
| `anchor` | `string` | キャプション（＝検索クエリ文・画像生成プロンプト） |
| `positive` | `Image` | レンダリングされた画像（＝検索ターゲット） |
| `category` | `string` | 被写体カテゴリ（緩い評価のオプションで使用） |
...
各クエリについて `query` / `target` / `rank_before_rerank` / `rank_after_rerank` / `top_k` を記録した配列。
...
|---|---|---|
| 画像生成 | SD-Turbo で実生成 | スタブ画像（ハッシュ由来の単色） |
| 埋め込みモデル | Qwen3-VL-Embedding-2B | clip-ViT-B-32（小型・CPU 可） |
| リランカー | Qwen3-VL-Reranker-2B | なし（スキップ） |
| 件数 | train 200 / eval 50 | train 8 / eval 4 |
