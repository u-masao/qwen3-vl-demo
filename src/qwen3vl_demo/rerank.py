...
def run_rerank(
    ...
    ``args`` を渡すと MLflow に記録する（Issue #9）。stage 全体を 1 つの run で包んで
    System Metrics（CPU/メモリ/GPU）・各工程の所要時間・VRAM ピークを残し、各バリアントの
    精度は子 run（nested）として記録する。None の場合（テスト等で直接呼ぶ場合）は記録しない。
    """
    ...
    # MLflow: stage 全体を 1 run で包む（System Metrics・所要時間・VRAM を記録）。
    # args 未指定（テスト等）のときは空の ExitStack となり、with は実質 no-op。
    stage = contextlib.ExitStack()
    if args is not None:
        enable_system_metrics()
        stage_params = {**args_to_params(args), **config_to_params(cfg), "top_k": top_k}
        stage.enter_context(
            start_run(
                run_name="rerank",
                params=stage_params,
                tags={"stage": "rerank", "variant": "stage"},
            )
        )
    ...
    with stage:
        ...
        for emb_name, emb_id in emb_variants.items():
            ...
            with Timer() as t:
                candidates[emb_name] = _retrieve_topk(
                    cfg, emb_id, queries, corpus_images, top_k
                )
        ...
        for rr_name, rr_id in rr_variants.items():
            ...
            with Timer() as t:
                reranked, peak_mib = _rerank_candidates(
                    cfg, rr_id, queries, corpus_images, candidates
                )
        ...
        # MLflow: stage run に所要時間・VRAM ピークを記録し、各バリアント精度を子 run に残す
        # （Retriever=rerank none と Reranker を同一 Experiment "evaluate" に並べる。Issue #9）。
        if args is not None:
